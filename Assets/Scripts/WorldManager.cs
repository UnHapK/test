...
public class WorldManager : MonoBehaviour
{
    ...
    {
        InitializeGame();
        var matchInstance = BackEndManager.GetInstance();
    ...
    private void PlayerDieEvent(SessionId index)
    {
        ...
        // 호스트가 아니면 바로 리턴
        if (!BackEndManager.GetInstance().IsHost())
        {
            return;
        }

        //if (BackEndManager.GetInstance().nowModeType == MatchModeType.TeamOnTeam)
        //{
        //    if (alivePlayer == 2)
        //    {
        ...
        //            {
        //                remainTeamNumber = BackEndManager.GetInstance().GetTeamInfo(player.Key);
    ...
    private void SendGameEndOrder()
    {
        ...
        foreach (SessionId session in BackEndManager.GetInstance().sessionIdList)
        {
        ...
        //BackEndManager.GetInstance().SendDataToInGame<GameEndMessage>(message);
    }
    ...
    {
        if (BackEndManager.GetInstance().sessionIdList == null)
        {
            // 현재 세션ID 리스트가 존재하지 않으면, 0.5초 후 다시 실행
            Invoke("SetPlayerInfo", 0.5f);
            return;
        }
        var gamers = BackEndManager.GetInstance().sessionIdList;
        ...
        //BackEndManager.GetInstance().SetPlayerSessionList(gamers);
        ...
        foreach (var sessionId in gamers)
        {
            ...
            if (BackEndManager.GetInstance().IsMySessionId(sessionId))
            {
                myPlayerIndex = sessionId;
                players[sessionId].Initialize(true, myPlayerIndex, BackEndManager.GetInstance().GetNickNameBySessionId(sessionId), statringPoints[index].w);
            }
            else
            {
                players[sessionId].Initialize(false, sessionId, BackEndManager.GetInstance().GetNickNameBySessionId(sessionId), statringPoints[index].w);
            }
            index += 1;
        }
    ...
    {
        if (BackEndManager.GetInstance() == null)
        {
            // 카운트 다운 : 종료
            //InGameUiManager.GetInstance().SetStartCount(0, false);
            return;
        ...
        if (BackEndManager.GetInstance().IsHost())
        {
            ...
            if (BackEndManager.GetInstance().IsSessionListNull())
            {
                Debug.Log("Player Index Not Exist!");
                // 호스트 기준 세션데이터가 없으면 게임을 바로 종료한다.
                foreach (var session in BackEndManager.GetInstance().sessionIdList)
                {
                    // 세션 순서대로 스택에 추가
                    gameRecord.Push(session);
                }
                GameEndMessage gameEndMessage = new GameEndMessage(gameRecord);
                //BackEndManager.GetInstance().SendDataToInGame<GameEndMessage>(gameEndMessage);
                return;
            }
        }
        SetPlayerInfo();
    ...
    IEnumerator StartCount()
    {
        ...
        for (int i = 0; i < START_COUNT + 1; ++i)
        {
            ...
            //BackEndManager.GetInstance().SendDataToInGame<StartCountMessage>(msg);
            yield return new WaitForSeconds(1); //1초 단위
        }

        // 게임 시작 메시지를 전송
        GameStartMessage gameStartMessage = new GameStartMessage();
        //BackEndManager.GetInstance().SendDataToInGame<GameStartMessage>(gameStartMessage);
    }
    ...
    public void OnGameOver()
    {
        ...
        //BackEndManager.GetInstance().MatchGameOver(gameRecord);
    }
    ...
    {
        if (BackEndManager.GetInstance().IsHost() == false)
        {
            //호스트만 수행
            return;
        }
    ...
    {
        if (BackEndManager.GetInstance().IsHost() == true)
        {
            //호스트면 리턴
            return;
        }
    ...
    private void ProcessSyncData(GameSyncMessage syncMessage)
    {
        ...
        //BackEndManager.GetInstance().SetHostSession(syncMessage.host);
    }
